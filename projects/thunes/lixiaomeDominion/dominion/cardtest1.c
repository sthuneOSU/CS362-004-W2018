...
int main() {
	...
	printf("player 0 has 2 more cards in hand ... ");
	printf(" (%d < %d) ",G.handCount[0], (testG.handCount[0]));
	if (asserttrue(G.handCount[0], (testG.handCount[0]), 3))
		printf("TEST PASSED\n");
	else 
		printf("TEST FAILED\n");	
	//test currentPlayer deck has decreased by 3
	printf("player 0 has 3 less cards in deck ... ");
	printf(" (%d > %d) ",G.deckCount[0], testG.deckCount[0]);
	if (asserttrue(G.deckCount[0], (testG.deckCount[0]), 5))
		printf("TEST PASSED\n");
	else 
		printf("TEST FAILED\n");
	//test currentPlayer discard has increased by 3
	printf("player 0 has 3 more cards in discard pile ... ");
	...
	if (asserttrue(G.discardCount[1], (testG.discardCount[1]), 1))
		...
		printf("TEST FAILED\n");		
	//test playedCardCount has increased
	printf("playedCardCount has increased by 1 ... ");
	printf(" (%d = %d) ",G.playedCardCount+1, testG.playedCardCount);
	if (asserttrue(G.playedCardCount+1, testG.playedCardCount, 1))
		printf("TEST PASSED\n");
	else 
		printf("TEST FAILED\n");
