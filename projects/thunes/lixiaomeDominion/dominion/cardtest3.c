...
int main() {
	...
	//test currentPlayer hand has increased by 1
	printf("player 0 hand size is unchanged ... ");
	printf(" (%d = %d) ",G.handCount[0], (testG.handCount[0]));
	if (asserttrue(G.handCount[0], (testG.handCount[0]), 1))
		printf("TEST PASSED\n");
	else 
		printf("TEST FAILED\n");	
	//test currentPlayer deck has decreased
	printf("player 0 deck size has decreased ... ");
	printf(" (%d-1 = %d) ",G.deckCount[0], testG.deckCount[0]);
	if (asserttrue(G.deckCount[0]-1, (testG.deckCount[0]), 1))
		printf("TEST PASSED\n");
	else 
		printf("TEST FAILED\n");
	//test currentPlayer discard has increased
	printf("player 0 has more cards in discard pile ... ");
	printf(" (%d+1 = %d) ",G.discardCount[0], testG.discardCount[0]);
	if (asserttrue(G.discardCount[0]+1, testG.discardCount[0], 1))
		printf("TEST PASSED\n");
	else 
		printf("TEST FAILED\n");	
	...
	//test actions have increased by 2
	printf("Test that actions have increased by 2 ... ");
	printf(" (%d+2=%d) " , G.numActions, testG.numActions);
	if (asserttrue(G.numActions+2, testG.numActions, 1))
		printf("TEST PASSED\n");
	else 
		printf("TEST FAILED\n");	
	return 0;
